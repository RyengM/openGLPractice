...
#define checkCudaErrors(val) UniSim::GPU::check_cuda( (val), #val, __FILE__, __LINE__)
...
namespace UniSim
{
	namespace GPU
	{
		...
		template <typename T>
		void check_results_autodesk(const T* const ref, const T* const gpu, size_t numElem, double variance,
			size_t tolerance)
		{
