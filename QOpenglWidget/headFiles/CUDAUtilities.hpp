#ifndef _CUDA_UTILITIES_HPP_
#define _CUDA_UTILITIES_HPP_
...
#define checkCudaErrors(val) UniSim::Helper::check_cuda( (val), #val, __FILE__, __LINE__)
...
namespace UniSim
{
    namespace Helper
    {
        ...
        template <typename T>
        void check_results_exact(const T* const ref, const T* const gpu, size_t numElem)
        {
        ...
        template <typename T>
        void check_results_eps(const T* const ref, const T* const gpu, size_t numElem, double eps1, double eps2)
        {
            ...
            {
                std::cerr << "Total percentage of non-zero pixel difference between the two images exceeds " << 100.0 *
                    eps2 << "%" << std::endl;
        ...
        template <typename T>
        void check_results_autodesk(const T* const ref, const T* const gpu, 
                                    size_t n_elem, double variance, size_t tolerance)
        {
            size_t n_bad_pixels = 0;
            ...
            if (n_bad_pixels > tolerance)
            {
                std::cerr << "Too many bad pixels in the image." << n_bad_pixels << "/" << tolerance << std::endl;
                exit(1);
            }
        }
    }
